...
/**
 * Get full container classes
 */
export const getContainerClasses = (): string => {
  const { container } = dropdownStyles
  return `${container.base} ${container.maxHeight}`
}
...
export const getSearchInputClasses = (): string => {
  return dropdownStyles.search.input
}

/**
 * Get list container classes
 */
export const getListContainerClasses = (): string => {
  const { list } = dropdownStyles
  return `${list.container} ${list.padding} ${list.spacing}`
}
